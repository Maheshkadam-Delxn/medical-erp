...
const SupplierPage = () => {
  ...
  const fetchSuppliers = async () => {
    ...
    try {
      ...
      const transformedSuppliers = suppliersArray.map((supplier) => ({
        ...
        licenseFileUrl: supplier.documents?.find((doc) => doc.name?.toLowerCase().includes("license"))?.url || null,
        gstFileUrl: supplier.documents?.find((doc) => doc.name?.toLowerCase().includes("gst"))?.url || null,
        joinDate: supplier.createdAt ? new Date(supplier.createdAt).toLocaleDateString() : "N/A",
      }))
  ...
  const handleAddSupplier = () => {
    router.push("/registration/supplier") // Correct path for supplier registration
  }
  ...
  const filteredSuppliers = React.useMemo(() => {
    ...
    return filter === "all"
      ? suppliers
      : suppliers.filter((s) => s.status && s.status.toLowerCase() === filter.toLowerCase())
  }, [suppliers, filter])
  ...
  const stats = React.useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: suppliersArray.filter((s) => s.status === "Blocked").length,
        icon: Shield,
  ...
  const handleApprove = async (supplierId) => {
    setLoading(true)
    try {
      const response = await fetch(`/api/superadmin/supplier/${supplierId}/approve-reject`, {
        method: "PATCH",
        headers: {
  ...
  const confirmRejection = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/superadmin/supplier/${supplierToReject}/approve-reject`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleBlockClick = (supplierId) => {
    setSupplierToBlock(supplierId)
    setShowBlockDialog(true)
  }

  // New function to confirm block with reason
  const confirmBlock = async () => {
    if (!supplierToBlock || !blockReason.trim()) return

    try {
      const response = await fetch(`/api/superadmin/supplier/${supplierToBlock}/block-unblock`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        setSuccessMessage("Supplier blocked successfully!")
        setShowSuccess(true)
        setTimeout(() => setShowSuccess(false), 3000)
  ...
  const handleUnblock = async (supplierId) => {
    try {
      const response = await fetch(`/api/superadmin/supplier/${supplierId}/block-unblock`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        setSuccessMessage("Supplier unblocked successfully!")
        setShowSuccess(true)
        setTimeout(() => setShowSuccess(false), 3000)
        await fetchSuppliers()
        closeModal()
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-green-500 flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-green-600 text-sm">Manage suppliers, approvals, and system oversight</p>
        </div>
                  ...
                  <tr key={supplier.id} className="hover:bg-green-50 transition-colors duration-150">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-gray-900">{supplier.companyName || "N/A"}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-700">{supplier.email || "N/A"}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Users className="h-4 w-4 text-gray-400" />
                        <span className="text-sm text-gray-700">{supplier.contactPerson || "N/A"}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <Phone className="h-4 w-4 text-gray-400" />
                        <span className="text-sm text-gray-700">{supplier.phone || "N/A"}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      ...
                      <div className="flex gap-2 justify-center">
                        ...
                        {supplier.status === "Blocked" ? (
                          <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            onClick={() => handleUnblock(supplier._id)}
                            className="p-2 text-green-600 hover:text-green-800 hover:bg-green-100 rounded-lg transition-colors"
                            title="Unblock"
                          >
                            <ShieldMinus size={18} />
                          ...
                          supplier.status !== "Rejected" && (
                            <motion.button
                              whileHover={{ scale: 1.05 }}
                              whileTap={{ scale: 0.95 }}
                              onClick={() => handleBlockClick(supplier._id)}
                              className="p-2 text-red-500 hover:text-red-700 hover:bg-red-100 rounded-lg transition-colors"
                              title="Block"
                            >
                              <Shield size={18} />
              ...
              >
                ...
                <div className="sticky top-0 bg-gradient-to-r from-green-500 to-emerald-600 px-6 py-4 flex justify-between items-center z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-green-100 text-sm">{selectedSupplier.companyName}</p>
                    </div>
                  </div>
                  <button
                    onClick={closeModal}
                ...
                <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
                  ...
                  >
                    ...
                    {selectedSupplier.status === "Blocked" && selectedSupplier.blockReason && (
                      <div className="mt-2">
                        <p className="text-sm font-medium text-red-800">Block Reason:</p>
                        <p className="text-sm text-red-700">{selectedSupplier.blockReason}</p>
                      </div>
                    )}
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      {selectedSupplier.approvedAt &&
                        ` • Approved: ${new Date(selectedSupplier.approvedAt).toLocaleDateString()}`}
                    </p>
                  </motion.div>
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-900">{selectedSupplier.companyName || "N/A"}</p>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-900">{selectedSupplier.contactPerson || "N/A"}</p>
                              <p className="text-sm text-gray-600">Contact Person</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <Mail className="h-4 w-4 text-blue-600" />
                            <p className="text-gray-700">{selectedSupplier.email || "N/A"}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <Phone className="h-4 w-4 text-blue-600" />
                            <p className="text-gray-700">{selectedSupplier.phone || "N/A"}</p>
                          </div>
                          <div className="flex items-start gap-3">
                            <MapPin className="h-4 w-4 text-blue-600 mt-1" />
                            <div>
                              <p className="text-gray-700">{selectedSupplier.address || "N/A"}</p>
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-900">{selectedSupplier.drugLicenseNumber || "N/A"}</p>
                              <p className="text-sm text-gray-600">Drug License Number</p>
                            </div>
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-900">{selectedSupplier.gstNumber || "N/A"}</p>
                ...
                <div className="sticky bottom-0 bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-t flex justify-between items-center">
                  <div className="text-sm text-gray-600">
                    ...
                    {selectedSupplier.updatedAt ? new Date(selectedSupplier.updatedAt).toLocaleString() : "N/A"}
                  </div>
          ...
          {showBlockDialog && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6"
              >
                <h3 className="text-xl font-bold text-red-800 mb-4">Block Supplier</h3>
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    placeholder="Enter the reason for blocking this supplier..."
                  />
                </div>
                <div className="flex justify-end gap-3">
                  <button
