...
const SupplierPage = () => {
  ...
  const handleApprove = async (supplierId) => {
    setLoading(true)
    try {
      const response = await fetch(`/api/supplier/${supplierId}/approve-reject`, {
  ...
  const confirmRejection = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch(`/api/supplier/${supplierToReject}/approve-reject`, {
        method: "PATCH", // Use PATCH method
        headers: {
          "Content-Type": "application/json",
        },
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredSuppliers.map((supplier) => (
                  <tr key={supplier._id} className="hover:bg-green-50 transition-colors duration-150">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium justify-center ${getStatusColor(supplier.status)}`}
                        >
