...
export default function SupplierRegistration() {
  ...
  const handleCategoryChange = (category) => {
    const updatedCategories = form.productCategories.includes(category)
      ? form.productCategories.filter((cat) => cat !== category)
      : [...form.productCategories, category]

    setForm({ ...form, productCategories: updatedCategories })

    if (errors.productCategories) {
      setErrors({ ...errors, productCategories: "" })
    }
  }
  ...
  const validateForm = () => {
    ...
    // Company information validation
      ...
      if (!form.supplierType.trim()) newErrors.supplierType = "Supplier type is required"
  ...
  const submitForm = async () => {
    ...
    Object.entries(form).forEach(([key, value]) => {
      if (key === "productCategories") {
        value.forEach((category) => formData.append("productCategories", category))
      } else {
    ...
    <div className="min-h-screen w-full flex font-sans">
      <div className="w-full lg:w-3/5 flex flex-col items-center justify-center px-4 lg:px-16 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800">Supplier Registration</h1>
