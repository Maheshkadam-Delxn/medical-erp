...
export async function PATCH(req, { params }) {
  try {
    ...
    const updatedChemist = await Chemist.findOneAndUpdate(
    ...
    if (!updatedChemist) {
      return NextResponse.json({ error: "Chemist not found with this User ID" }, { status: 404 })
    }

    console.log(`Chemist with ID ${_id} status updated to: ${isApproved ? "Approved" : "Rejected"}`)

    return NextResponse.json({
      success: true,
      data: updatedChemist,
      message: `Chemist with ID ${_id} has been ${isApproved ? "approved" : "rejected"}.`,
    })
  } catch (error) {
    console.error("Error updating chemist status:", error)
    ...
    return NextResponse.json({ error: "Failed to update chemist status", details: error.message }, { status: 500 })
  }
}
