...
const ProductManagement = () => {
  ...
  const fetchExpiryAlerts = async () => {
    try {
      const response = await fetch('/api/supplier/product/alerts/expiry');
      if (!response.ok) {
  ...
  const handleEdit = (product) => {
    setCurrentProduct({ 
      ...product,
      quantity: product.batches.reduce((sum, batch) => sum + (batch.quantity || 0), 0)
    });
    setIsModalOpen(true);
  };
  ...
  const saveProduct = async () => {
    // Basic validation
    if (!currentProduct.name || !currentProduct.category || !currentProduct.code) {
      alert('Please fill in required fields: Name, Category, and Product Code');
      return;
    }

    // Validate selling price
    if (parseFloat(currentProduct.sellingPrice) < parseFloat(currentProduct.purchasePrice)) {
      alert('Selling price must be greater than or equal to purchase price');
      return;
    }

    // Validate batches for active products
    if (currentProduct.active && (!currentProduct.batches || currentProduct.batches.length === 0)) {
    ...
    for (const batch of currentProduct.batches) {
      if (!batch.batchNumber || !batch.quantity || isNaN(batch.quantity) || 
          !batch.manufacturerDate || !batch.expiryDate) {
        alert('Please ensure all batches have valid batch number, quantity, and dates');
        return;
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(currentProduct._id ? 'Product updated successfully' : 'Product created successfully');
      } else {
        const errorData = await response.json();
        alert(`Error: ${errorData.message}`);
      }
  ...
  const getTotalStock = (product) => {
    return product.batches.reduce((sum, batch) => sum + (batch.quantity || 0), 0);
  };

  // Check if product is low on stock
  const isLowStock = (product) => {
    return getTotalStock(product) <= (product.reorderLevel || 0);
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCurrentProduct({...currentProduct, reorderLevel: Math.max(0, parseInt(e.target.value) || 0)})}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCurrentProduct({...currentProduct, purchasePrice: Math.max(0, parseFloat(e.target.value) || 0)})}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCurrentProduct({...currentProduct, sellingPrice: Math.max(0, parseFloat(e.target.value) || 0)})}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={currentProduct.batches.reduce((sum, batch) => sum + (batch.quantity || 0), 0)}
                    readOnly
                  />
                </div>
              </div>
                          ...
                          <tr key={index}>
                            ...
                            <td className="px-4 py-2 whitespace-nowrap">
                              <input
                                ...
                                onChange={(e) => handleBatchUpdate(index, 'quantity', Math.max(0, parseInt(e.target.value) || 0))}
                                className="w-full px-2 py-1 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-green-500 focus:border-transparent"
                                required
                              />
                            </td>
